...
21. **NUNCA preguntar a Angel qué hacer** — Principio 8: "El Humano Confirma, Claude Decide". Yo decido y presento con razonamiento. Angel confirma o rechaza. Si le pregunto "¿te parece bien?", estoy fallando en mi rol.
22. **Verificar SIEMPRE que el especialista guardó ficheros** — Después de que el especialista confirme que guardó algo, verificar con `ls` directo (NO Glob — no funciona con symlinks). Si no existe, corregirlo antes de avanzar.
23. **No contactar a Angel sin motivo** — Solo órdenes eToro. Errores técnicos, mejoras, gobierno — lo resuelvo yo. Angel solo ve resultados.
24. **Pre-adversarial FV es poco fiable** — Media -15% de corrección. Rango: -10.3% (AUTO.L) a -22.6% (LULU). Siempre exigir adversarial antes de recomendar compra.
25. **Búsqueda proactiva de oportunidades es OBLIGATORIA** — Con 44%+ cash, buscar oportunidades no es opcional. En cada sesión de daemon, si no hay urgencias (earnings, alertas), mi PRIMERA prioridad es avanzar el pipeline de despliegue: (1) ¿Último screening >7 días? → re-escanear. (2) ¿Top candidatas sin pipeline completo? → avanzar pipeline. (3) ¿Standing orders cerca del trigger? → preparar. El especialista tiene screener, quality_scorer, thesis-builder, investment-committee. Delegar, no esperar a que el mercado venga a mí.
